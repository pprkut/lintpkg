...
@test "Check logs no error when correct owner and group for /bin" {
  ...
  PKG_DETAILED_LISTING=$(create_detailed_tar_listing $BATS_TEST_TMPDIR | sed "s|$USER/$(id -gn)|root/root|g")
...
@test "Check logs no error when owner is whitelisted (daemon) for file in /usr/bin" {
  ...
  WORKING_DIR=$BATS_TEST_TMPDIR
  PKG_DETAILED_LISTING=$(create_detailed_tar_listing $BATS_TEST_TMPDIR | sed "s|$USER/$(id -gn)|root/root|g" | sed "/usr\/bin\/foo/s|root/root|daemon/daemon|g")
...
@test "Check logs no error when owner is whitelisted (uucp) for file in /usr/bin" {
  ...
  WORKING_DIR=$BATS_TEST_TMPDIR
  PKG_DETAILED_LISTING=$(create_detailed_tar_listing $BATS_TEST_TMPDIR | sed "s|$USER/$(id -gn)|root/root|g" | sed "/usr\/bin\/foo/s|root/root|uucp/uucp|g")
...
@test "Check logs no error when owner is whitelisted (daemon) for file in /usr/sbin" {
  ...
  WORKING_DIR=$BATS_TEST_TMPDIR
  PKG_DETAILED_LISTING=$(create_detailed_tar_listing $BATS_TEST_TMPDIR | sed "s|$USER/$(id -gn)|root/root|g" | sed "/usr\/sbin\/foo/s|root/root|daemon/daemon|g")
...
@test "Check logs no error when owner is whitelisted (uucp) for file in /usr/sbin" {
  ...
  WORKING_DIR=$BATS_TEST_TMPDIR
  PKG_DETAILED_LISTING=$(create_detailed_tar_listing $BATS_TEST_TMPDIR | sed "s|$USER/$(id -gn)|root/root|g" | sed "/usr\/sbin\/foo/s|root/root|uucp/uucp|g")
